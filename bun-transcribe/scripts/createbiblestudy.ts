...
export async function createBibleStudy(video_id: string) {
    console.log(`Generating bible study for video ID: ${video_id}`);
    ...
    messages: [
      {
        role: 'user',
        content: `GOAL:
- Transform the provided notes into a Discovery-Based Bible Study Guide.
- The guide must act as a facilitator, prompting users to find truths in Scripture themselves rather than providing a summary of answers.

CONSTRAINTS:
- DO NOT provide explanations or summaries of the "answers." 
- Use investigative and reflective questioning.
- Focus on "Observation, Interpretation, and Application."
- Language should be simple, clear, challenging yet spiritually inviting.

STRUCTURE & FORMAT:
1. **Title**: A provocative, theme-based title.
2. **The Anchor Text**: Identify the primary passage(s) the reader must have open.
3. **Contextual Investigation**: Provide 2-3 questions that prompt the reader to research the historical or biblical context of the passage (e.g., "Look up the city of Philippi; why was Paul's message here so counter-cultural?").
4. **Discovery Questions (Observation & Interpretation)**: 
   - Use the main points of the notes to generate questions that force the reader to look back at the text.
   - Example: Instead of "God is faithful," ask "Based on verses 4-8, what specific actions of God demonstrate His character in a crisis?"
5. **Deep Reflection**: 3-4 "Heart-Work" questions designed for silence and introspection (don't specify how long the silence lasts, let them decide). These should be open-ended and focus on the "why" of the reader's current spiritual state.
6. **The Challenge (Application)**: Instead of a "To-Do" list, provide a single, significant "Discovery Challenge" that requires the reader to live out a principle from the notes this week.
7. **Scripture for Further Study**: List all verses from the notes to be used as a personal reading plan.
   - Format: "Book Chapter:Verse-Verse".
   - Never abbreviate book names.
...
`,
    ...
    Bun.write(`./bible-studies/${video_id}_biblestudy.md`, markdown);
    // console.log(completion?.choices[0]?.message.content);
    console.log(`Bible study generated for video ID: ${video_id}`);
