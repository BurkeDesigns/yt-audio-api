#!/usr/bin/env python3
"""
Audio Quick Transcribe - Standalone Script

Fast audio transcription from a local file using NeMo Parakeet.
Supports infinite length audio through chunking with silence removal.

Usage:
    python audio_quick_transcribe_cuda.py "input_audio.wav" "output.json"
    
Example:
    python audio_quick_transcribe_cuda.py "my_recording.mp3" "transcript.json"
"""
...
def main():
    """Main entry point."""
    if len(sys.argv) < 3:
        print("Usage: python audio_quick_transcribe_cuda.py <AUDIO_FILE_PATH> <OUTPUT_JSON_PATH>")
        print("\nExample:")
        print("  python audio_quick_transcribe_cuda.py podcast.mp3 transcript.json")
        sys.exit(1)
    ...
    print(f"\n{'='*70}")
    print(f"🎙️ Audio Quick Transcribe")
    print(f"{'='*70}")
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="audio_transcribe_"))
    
    try:
        total_start = time.time()
        
        # Get duration for metadata
        audio_duration = ffprobe_duration_seconds(str(audio_path))
